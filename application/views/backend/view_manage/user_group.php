                            ...
                            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                                ...
                                <?php foreach($get_all as $key=>$val): ?>
                                    <tr class="">
                                        ...
                                        <td><?= $val->LEVEL ?></td>
                                        <?php //$text = str_replace('src="', 'src="'.base_url(), $val->DESCRIPTION); ?>
                                        <td><?= $val->DESCRIPTION ?></td>
                                        <td>
                                            <button class="btn btn-primary" onclick="window.location='<?= base_url().'backoffice/'.$this->uri->segment(2).'/edit/'.$val->ID_GROUP ?>';">
                                                <i class="icon-pencil icon-white"></i>
                                                Edit
                                            </button>
                                            <a href="#deletePopUp" onclick="javascript:setNameDelete('<?= $val->GROUP_NAME ?>','<?= $val->ID_GROUP ?>')" data-toggle="modal" class="btn btn-danger"><i class="icon-remove icon-white"></i>Delete</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
