...
//                                            $text=(string)$text;
//                                            echo substr_replace($text, '...', 30, strlen($text));
                                            ?>
<!--                                        </td>-->
                                        <td><img src="<?= base_url().'upload/_thumbs/Images/'.$val->IMAGES ?>" /></td>
                                        <td>
                                            <?php
                                            $query_page=$this->interactive_model->get_page_by_id($val->LINK);
                                            foreach($query_page as $val2){
                                                echo $val2->NAME_PAGE;
                                            }
                                            ?>
                                        </td>
                                        <td><?= date('Y-m-d H:i:s', $val->DATE) ?></td>
                                        <td class="center">
                                        ...
                                        <td>
                                            <button class="btn btn-primary" onclick="window.location='<?= base_url().'backoffice/'.$this->uri->segment(2).'/edit/'.$val->ID_INTERACTIVE ?>';">
                                                <i class="icon-pencil icon-white"></i>
                                                Edit
                                            </button>
                                            <a href="#deletePopUp" onclick="javascript:setNameDelete('<?= $val->NAMA ?>','<?= $val->ID_INTERACTIVE ?>')" data-toggle="modal" class="btn btn-danger"><i class="icon-remove icon-white"></i>Delete</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
